...
Version 20250521 +
Copyright © Loongson Technology 2025. All rights reserved.
...
|20250521
|added `_Float16`, `__bf16` and `_BitInt(N)` definitions; fixed discriptive error
...
** An fp16 / fp32 / fp64 argument is passed in an FAR if there is one available.
Otherwise, it is passed in a GAR, or on the stack if none of the GARs are
available. When passed in registers or on the stack, fp16 / fp32 / fp64 arguments
